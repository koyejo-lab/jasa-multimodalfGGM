...
k1 = fmri.shape[2]
tr_fmri = np.zeros((fmri.shape[0],fmri.shape[1]*k1))


for n in range(fmri.shape[0]):
    for i in range(fmri.shape[1]):
        tr_fmri[n, i*k1:(i+1)*k1] = fmri[n,i,:]



fmri_mean = np.mean(tr_fmri, axis=0)
tr_fmri = tr_fmri - fmri_mean
...
te_fmri = np.zeros((fmri.shape[0],fmri.shape[1]*k1))

for n in range(fmri.shape[0]):
    for i in range(fmri.shape[1]):
        te_fmri[n, i*k1:(i+1)*k1] = fmri[n,i,:]
...
data = np.load('./result_experiment/experiment_result_fmri_movie_all_first23.npy', allow_pickle=True)
...
fmri_supp = np.zeros((p*k1, p*k1)) #pk x pk
fused_supp = np.zeros((p*k1,p*k1))
##consturct support:
for i in range(p):
    for j in range(p):
        if adj[i,j] == 1:
            subm = np.zeros((k1,k1))
            sub_sup = tildeB[i,:,j*k1:(j+1)*k1]
            idx = np.where(sub_sup != 0)
            if idx[0].size>0:
                subm[:,:] = 1
            fmri_supp[i*k1:(i+1)*k1, j*k1:(j+1)*k1] = subm
            fused_supp[i*k1:(i+1)*k1, j*k1:(j+1)*k1] = subm
plt.imshow(fmri_supp)
plt.savefig('supp_movie_fmri.png')
print(np.max(fmri_supp))
#fmri_lam = (1-fmri_supp)*1e9
#id = np.where(fmri_lam==0)
#fmri_lam[id] = .1

#load the estimated operator A
...
p = adj.shape[0]
k = A1.shape[0]
supp = np.zeros((p*k1, p*k1)) #pk x pk
...
for i in range(p):
    for j in range(p):
        if adj[i,j] == 1:
            ...
            if idx[0].size>0:
                subm[:,:] = 1
            supp[i*k1:(i+1)*k1, j*k1:(j+1)*k1] = subm
            fused_supp[i*k1:(i+1)*k1, j*k1:(j+1)*k1] = subm
...
#umcomment the following to run cv for selecting fmri_l
#pool = Pool(10)
#start_time1 = time.time()
#results = pool.starmap(cv_lasso, zip(repeat(fmri_supp), repeat(tr_fmri), repeat(5), lambdas))
#end_time1 = time.time()
#print("Total Execution time:", end_time1 - start_time1)
#pool.close()
#min_idx = np.argmin(results)
#fmri_l = lambdas[min_idx]


fmri_l = 0.5
#print('[fmri] lambda:', fmri_l)

fmri_lam = (1-fmri_supp)*1e9
idx = np.where(fmri_lam==0)
fmri_lam[idx] = fmri_l


#initiate estimator
estimator = QuicGraphicalLasso(score_metric="log_likelihood", lam=fmri_lam, verbose=1)
estimator.fit(tr_fmri)
prec = estimator.precision_
np.save('fmri_individual.npy', prec)
print(slogdet(prec))
print(np.log(det(prec)))


insample = compute_loglikelihood(tr_fmri, prec)
print('[fmri] in-sample loglikelihood:', insample)
outsample = compute_loglikelihood(te_fmri, prec)
print('[fmri] out-of-sample loglikelihood', outsample)
...
for i in range(n_fold):
    ...
    estimator = QuicGraphicalLasso(score_metric="log_likelihood", lam=fmri_lam, verbose=1)
    estimator.fit(tr_fmri[select_t])
    prec = estimator.precision_
    #print(prec)
    #print(slogdet(prec))
    ...
    insample = compute_loglikelihood(tr_fmri[select_t], prec)
...
print('[fmri] average in-sample loglikelihood', np.mean(acc_lln), np.std(acc_lln))
print('[fmri] average out-of-sample loglikelihood', np.mean(occ_lln), np.std(occ_lln))
...
#pool = Pool(10)
#start_time1 = time.time()
#results = pool.starmap(cv_lasso, zip(repeat(supp), repeat(tr_fmri), repeat(5), lambdas))
...
print(slogdet(prec))
print(np.log(det(prec)))
np.save('fmri_latent.npy', prec)


insample = compute_loglikelihood(tr_fmri, prec)
print('[joint] in-sample loglikelihood:', insample)
outsample = compute_loglikelihood(te_fmri, prec)
print('[joint] out-of-sample loglikelihood', outsample)
...
for i in range(n_fold):
    ...
    occ_lln.append(outsample)
    acc_lln.append(insample)
print('[joint] average in-sample loglikelihood', np.mean(acc_lln), np.std(acc_lln))
print('[joint] average out-of-sample loglikelihood', np.mean(occ_lln), np.std(occ_lln))
...
#pool = Pool(10)
#start_time1 = time.time()
#results =pool.starmap(cv_lasso, zip(repeat(fused_supp), repeat(tr_fmri), repeat(5), lambdas))
#end_time1 = time.time()
#print("Total Execution time:", end_time1 - start_time1)
#pool.close()
#min_idx = np.argmin(results)

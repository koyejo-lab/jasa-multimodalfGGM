...
graph.filename <- paste("graph_",cov_name,"_p", p, "_N", n, "_k",k,sep="")
data.filename <- paste("data_",cov_name,"_p", p, "_N", n, "_k",k,sep="")
...
for(m in 1:M){
    ...
    #structured noise
    #y <- rsparsematrix(p*km.gen[m], p*km.gen[m], 0.05, 
    #          symmetric = TRUE)
    #eig_y <- eigen(y,symmetric = TRUE) 
    #id <- which(eig_y$values>0)
    #N_list[[m]]  <- as.matrix( eig_y$vectors[,id] %*% diag(eig_y$values[id]) %*% t(eig_y$vectors[,id])) *0.1

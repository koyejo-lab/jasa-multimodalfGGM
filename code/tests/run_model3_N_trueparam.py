...
N_list = [275, 305, 344, 393, 458, 550, 688, 917, 1376, 2752, 13761]   #for p=50
#N_list = [324, 360, 405, 462, 540, 648, 810, 1080, 1620, 3240,16200]  #for p=100
#N_list = [352, 391, 440, 503, 587, 705, 881, 1175, 1762, 3525, 17626] #for p=150
tpr_fpr = np.zeros((len(N_list), 2))

def run_single_model_N(N):

    path=args.filepath
    filename= "graph_{}_p{}_N{}{}.RData".format(args.type, args.p, N, args.noise)
    true_param = load_Rparams(path, filename)
    filename= "data_{}_p{}_N{}{}.RData".format(args.type, args.p, N, args.noise)
    ...
    tpr, fpr = model.fit( X, tol, true_A=true_param['Am'], true_tildeB = true_param['tildeB'], evaluate=True, iniA=true_param['Am'], iniB=None, thre=args.thre)
...
pool = Pool(min(cpu_count()-10, len(N_list)))
...
savefile = "model3_{}_p{}_{}_rate{}{}{}_ss{}_trueparam_run2".format(args.type, args.p, args.noise, 

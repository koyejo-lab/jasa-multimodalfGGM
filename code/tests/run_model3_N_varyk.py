...
parser.add_argument('--N', type=int, default=324)
...
N_list = [324,648]
k_list = [3,5,7,9]
tpr_fpr = np.zeros((len(k_list), 2))

def run_single_model_k(k):

    path=args.filepath
    filename= "graph_{}_p{}_N{}{}_k{}.RData".format(args.type, args.p, args.N, args.noise,k)
    true_param = load_Rparams(path, filename)
    filename= "data_{}_p{}_N{}{}_k{}.RData".format(args.type, args.p, args.N, args.noise,k)
...
pool = Pool(min(15, len(k_list)))

#pool = Pool(1)
start_time1 = time.time()
results = pool.map(run_single_model_k, k_list)
end_time1 = time.time()
...
savefile = "model3_{}_p{}_N{}_alpha{}_rate{}{}{}_ss{}_ss_varyk_run2".format(args.type, args.p, args.N, alpha,
